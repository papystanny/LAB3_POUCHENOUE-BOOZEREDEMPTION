...
        public float speed = 2f; private float height; //Height of the platform
...
            }     // Update is called once per frame
        ...
        void Update()
        {
            ...
            {
                if (!canChange)
                    return; if (transform.position.y > posYDown)
                {
                    transform.position -= Vector3.up * Time.deltaTime * speed;
                }
                else if (!isWaiting)
        ...
        IEnumerator WaitToChange(float time)
        {
            ...
            isWaiting = false;
            isDown = !isDown; if (isRandom && !isDown) //If is wall up and is random
            {
